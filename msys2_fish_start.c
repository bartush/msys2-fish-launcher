...
int wmain(int argc, wchar_t* argv[])
{
    ...
    *tmp++ = L'i';
	...
	while (code < 0 && buflen < 8192)
		...
		//msystem = L"UCRT64";
		code = swprintf(buf, buflen, L"%ls\\usr\\bin\\mintty.exe -i '%ls' -o 'AppLaunchCmd=%ls' -o 'AppID=MSYS2.Shell.%ls.%d%ls' -o 'AppName=MSYS2 %ls Shell' -t 'MSYS2 %ls Shell' --store-taskbar-properties -- %ls %ls", msysdir, exepath, exepath, msystem, APPID_REVISION, msysdirhash, msystem, msystem, /*argc == 1 ? L"-" :*/  L"/usr/bin/sh -lc '\"$@\"' sh", L"fish"/*args*/);
		buflen *= 2;
	    }
	if (code < 0)
	    {
